<template>
    <div class="merchants">
        ...
        <div>
            <van-checkbox @change="changeCheack" v-model="checkedtwo" class="feaCheckbox">同意<span @click.stop="contract">《平台服务合同》</span>和<span @click.stop="discounts">《隐私条约》</span></van-checkbox>
            <van-checkbox @change="changeCheack" v-model="checkedone" class="feaCheckbox">同意<span @click.stop="discount">《代运营合同》</span></van-checkbox>
        </div>
...
export default {
    data(){
        return{
            ...
            idcardFaceStatus:0,//判断是否有上传身份证正
            idcardBack:idcardBack,//身份证反
            idcardBackStatus:0,//判断是否有上传身份证反
            chartered:chartered,//营业执照
            charteredStatus:0,//判断是否有上传营业执照
    ...
    created(){
        ...
        if(edit == 'on'){
            ...
            if(localStorage.getItem('companyAddress') != null || localStorage.getItem('companyAddress') != ''){
                    this.address = localStorage.getItem('companyAddress');
            }else if(localStorage.getItem('getLocation') != null || localStorage.getItem('getLocation') != ''){
                let addressCode = JSON.parse(code_sn);
                this.address = addressCode.province + addressCode.city + addressCode.district;
            }
            if(localStorage.getItem('DetailAddress') != null || localStorage.getItem('DetailAddress') != ''){
                    this.addressInfo = localStorage.getItem('DetailAddress');
            }else if(localStorage.getItem('getLocation') != null || localStorage.getItem('getLocation') != ''){
                let addressCode = JSON.parse(code_sn);
                this.addressInfo = addressCode.street + addressCode.street_number;
            }
            if(localStorage.getItem('linkman') != null || localStorage.getItem('linkman') != ''){
                    this.fullName = localStorage.getItem('linkman');
                }
            if(localStorage.getItem('telNumber') != null || localStorage.getItem('telNumber') != ''){
                    this.contacTinformation = localStorage.getItem('telNumber');
                }
            if(localStorage.getItem('scode') != null || localStorage.getItem('scode') != ''){
            ...
            if(localStorage.getItem('regionSn') != null || localStorage.getItem('regionSn') != ''){
            ...
            this.$post('/?c=user&a=upgradeMerchantsInfo',{
            }).then((res)=>{
                switch (res.code) {
                    case 0:
                        //地址 区域处理
                        this.enterpriseName = res.data.companyName;         //公司名称
                        this.address = res.data.address;                   //公司下拉框（省市区）
                        this.addressInfo = res.data.company_address;         //详细所在地
                        this.contacTinformation = res.data.contactPhone;    //联系方式
                        this.fullName = res.data.contactName;               //联系人
                        this.regionSn = res.data.region_sn;                 //地址
                        this.businessLicense = res.data.business_license_id;//营业执照号

                        if(res.data.legal_person_fileImg != ''){
                            this.idcardFace = res.data.legal_person_fileImg;    //身份证正
                            this.idcardFaceStatus = 1;
                        }
                        if(res.data.legal_person_fileImg_back != ''){
                            this.idcardBack =res.data.legal_person_fileImg_back;//身份证反
                            this.idcardBackStatus = 1;
                        }
                        if(res.data.license_fileImg != ''){
                            this.chartered = res.data.license_fileImg;          //营业执照
                            this.charteredStatus = 1;
                        }
                        ...
                        for(let key in cat){
                            enterItem.cat_id = key;
                            enterItem.cat_name = cat[key];
                        }
                        let cat_son = res.data.cat_son;
                        for(let key in cat_son){
                            enterlable[key] = cat_son[key];
                        }
                        
                        if(enterItem!==''&&enterItem!==null){ 
                            this.onetitle=enterItem.cat_name
                        }
                        if(enterlable!==''&&enterlable!==null){
                            let names='';
                            let ids='';
                            for(var key in enterlable){
                                names+=('|'+enterlable[key]);
                                ids+=(key+',');
                            }
                            ids = ids.substring(0,ids.length-1);
                            this.idarr=ids;
                        ...
                        localStorage.setItem('enterItem',JSON.stringify(enterItem));
                        localStorage.setItem('enterlable',JSON.stringify(enterlable));

                        localStorage.setItem('regionSn',res.data.region_sn);
                        localStorage.setItem('companyAddress',res.data.address);
                        localStorage.setItem('companyName',res.data.companyName);
                        localStorage.setItem('DetailAddress',res.data.company_address);
                        localStorage.setItem('linkman',res.data.contactName);
                        localStorage.setItem('telNumber',res.data.contactPhone);
                        
                        localStorage.setItem('idcardFace',res.data.legal_person_fileImg);
                        localStorage.setItem('idcardBack',res.data.legal_person_fileImg_back);
                        localStorage.setItem('chartered',res.data.license_fileImg);
                        localStorage.setItem('businessLicense', res.data.business_license_id);
    ...
    /*  */
        ...
        //获取短信验证码
        getCode(){
            if (!this.vtCustomerPhone()) return; 
    ...
    /*  */
        wxSetUsenImge(imgType){
            ...
            if(_this.imgCleck){
                ...
                wxUpload({},_this,imgType,100,(e)=>{
                    if(imgType == 'card1'){
                        _this.idcardFace = e;
                        _this.idcardFaceStatus = 1;
                        localStorage.setItem('idcardFace',e);
                    }
                    else if(imgType == 'card2'){
                        _this.idcardBack = e;
                        _this.idcardBackStatus = 1;
                        localStorage.setItem('idcardBack',e);
                    }
                    else if(imgType == 'card3'){
                        _this.chartered = e;
                        _this.charteredStatus = 1;
                        localStorage.setItem('chartered',e);
                    }
                    _this.imgCleck = true;
                },(cancel)=>{
                    _this.imgCleck = true;
