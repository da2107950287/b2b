<template>
    <div style="position: relative;overflow-x: hidden;height:auto;">
        ...
        <div class="new-goods-box">
            ...
            <div v-if="showList == 1">
                <goodsLi class="goodsLi" v-for="(item,index) in goodsList" type="newGoods" :key="index" @showCart="shoppingInCart($event,item)" :item="item" :page="gpage" :lock="lock" :index="index"></goodsLi>
        ...
        <div id="flyItem"><img :src="eleParabolaImg" alt=""></div><!-- 抛物线 -->
    ...
    methods:{
        ...
        shoppingInCart(res){
            ...
            let data = {
                ...
                wholesale_id: res.item.wId
            }
            
            if(this.is_animate){
                this.is_animate = false;
                if(!this.$appFunc.prototype.isEmpty(res.item.goods_thumb)){
                    this.eleParabolaImg = res.item.goods_thumb;
                }else{
                    this.eleParabolaImg = res.item.goods_img;
                }

                let img = $($event.target).parents('.goods-content').prev().find('img');
