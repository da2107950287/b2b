			...
			<div class="list">
				<!-- <div class="list-item">
				 	...
				 	<div class="list-right">
				 		<div @click="goToPage('')">本月推广分成: <span class="red">90.21</span>元 <van-icon name="arrow" /></div>
				 		<div>本月订单次数: 5次</div>
				 	</div>
				</div> -->
				...
				<div class="list-item" v-for="(item, index) in dataList" :key="index">
					...
					<div class="list-y"></div>
					<div class="list-right">
						<div @click="goToPage(item.user_id)">本月推广分成: <span :class="{'red': item.develop_status == 2}">{{item.all_amount}}</span>元 <van-icon name="arrow" /></div>
						<div>本月订单次数: {{item.all_num}}次</div>
					</div>
				</div>
			</div>
...
export default {
	...
	methods: {
		...
		getList(){
			this.lock = 2;
            this.$post('/?c=wholesaleCommission&a=enterprise',{
            	page: this.page,
            	limit: this.limit
            })
            .then((res)=>{
