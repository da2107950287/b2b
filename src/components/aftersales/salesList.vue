<template>
    <div>
        <div v-if="show === 2" class="salesList sales_list_box">
            <div v-for="(item,index) in lists" :key="index" class="sales_list_li">
                <div class="sales_list_cont" @click="toSalesDetails(item.after_sn)">
                    <div class="sales_list_order_no fz_14 color_424242 dflex">
                        <div class="list_order_no">服务单号：{{item.after_sn}}</div>
                        <div class="list_order_status color_EE2D2D">{{getStatus(item.status)}}</div>
                    </div>
                    <div class="goods_box" v-for="(items,index) in item.goods" :key="index">
                    ...
                    <div class="button_box fz_14">
                        <div class="dflex">
                            <van-button @click.stop="toSalesDetails(item.after_sn)">查看详情</van-button>
                        </div>
                        <div class="dflex" style="display:none;">
                            <van-button @click.stop="logisticsDetails(item.after_sn)">查看进度</van-button>
                            <van-button @click.stop="toSalesDetails(item.after_sn)" style="margin-left:10px;">查看详情</van-button>
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    ...
    export default {
        ...
        methods:{
            ...
            getStatus(status){
                switch(status){//售后状态  0待确定 1商家已确定  2平台已确定 3用户取消 4商家已驳回 5用户申述 6拓展介入 7拓展驳回 8拓展确认并修改售后重新提交 9平台申述确认
                    case 0:
                        return '待确认'
                        break;
                    case 1:
