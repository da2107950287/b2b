<template>
    <div class="group-row-box enterLabelBox">
       <!-- 顶部 -->
        <navbar type="both" title="选择分类" @clickleft="onClickLeft" @clickRight="onClickRight" rightTxt="确定"></navbar>
        <van-row class="group-row">
            <div class="dflex">
                <van-col span="8">
                    <div class="enterleft">
                        <van-badge-group :active-key="activeKey" @change="onChange" >
                                <van-badge  v-for="(item,index) in oneMenu" :key="index" :title="item.cat_name" @click="everyItem(item)"/>
                        </van-badge-group>
                    </div>
                </van-col>
...
export default {
    ...
    created() {
        ...
        this.$post('/?a=category',{
            ...
            // 获取二级菜单
            this.$post('/?a=category',{
                parent_id:this.selectedCat1.cat_id
            }).then((res)=>{
                ...
                if(this.chose == 'radioDom'){
                    ...
                    let twoMenu=res.data
                    for(let key in twoMenu){
                        twoMenu[key] = {...twoMenu[key],active:1}
                    }
                    this.twoMenu = twoMenu;
    ...
    methods:{
       ...
       onClickRight(){
        ...
        onClickAll(){
            if(this.checkAll == true){
                for(let key in this.twoMenu){
                    ...
                    delete this.selectedCat[this.twoMenu[key].cat_id];
                }
            }else{
                for(let key in this.twoMenu){
                    this.checkAll = true;
                    this.twoMenu[key].active = 1;
                    this.selectedCat[this.twoMenu[key].cat_id] = this.twoMenu[key].cat_name;
                }
            }
        },
        // 点击二级菜单触发
        checkRadio(item,index){
            ...
            if(item.active == 1){
                item.active = 0;
                delete this.selectedCat[item.cat_id];
            }else{
                item.active = 1;
                if(this.chose == 'radioDom'){
                    this.selectedCat[item.cat_id] = {
                        name:item.cat_name,
                        unit:item.unit
                    };
                }else{
                    this.selectedCat[item.cat_id] = item.cat_name;
                }
